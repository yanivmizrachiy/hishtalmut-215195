...
function mathify(s=''){
  return String(s).split(/(`[^`]*`)/g).map(part => {
    if (part.startsWith('`') && part.endsWith('`')) return `<span class="math">${esc(part.slice(1,-1))}</span>`;
    return esc(part).replace(/\b([xy]|m)\b/g,'<span class="math">$1</span>');
  }).join('');
}
...
function axesSvg(g){
  ...
  for(const raw of g.points||[]){
    ...
    pts+=`<circle class="point" cx="${x(pt.x)}" cy="${y(pt.y)}" r="4"/>`;
    if(pt.label) pts+=`<text class="point-label" x="${x(pt.x)+8}" y="${y(pt.y)-8}">${esc(pt.label)}</text>`;
    else if(g.showCoordinates!==false) pts+=`<text x="${x(pt.x)+7}" y="${y(pt.y)-7}">(${pt.x},${pt.y})</text>`;
  }
