export const page={
  page:67,
  chapter:25,
  kicker:'פרק 25 · שאלות מיצ״ב — פונקציה קווית',
  title:'מיצ״ב — שיפוע, עלייה ומקבילות',
  subtitle:'סימן השיפוע → נקודה נוספת → שיפוע זהה',
  rule:'השיפוע הוא מספר: שיפוע חיובי מתאים לגרף עולה, שיפוע שלילי לגרף יורד, ושני ישרים שונים בעלי אותו שיפוע הם מקבילים.',
  sourceRefs:[
    'drive:1JBzDY_ntI7Or2mqAVT1secDAO1FVd2Q4 — מקבץ פונקציה קוית מייצב תשעא-תשעו.docx, שאלות 1,8,16'
  ],
  questions:[
    {
      id:'MZ06-P67-Q1',family:'MZ06',level:4,responseSpace:'choice-mark',
      stem:'אחת מהפונקציות שלפניכם היא פונקציה יורדת. סמנו אותה.',
      choices:['`y=-6`','`y=-4x+1`','`y=3x-5`','`y=9x`'],
      sourceRef:'drive:1JBzDY_ntI7Or2mqAVT1secDAO1FVd2Q4, מקבץ פונקציה קוית מייצב תשעא-תשעו.docx, question 1 — choose the decreasing function from y=-6, y=-4x+1, y=3x-5, y=9x'
    },
    {
      id:'MZ07-P67-Q2',family:'MZ07',level:5,responseSpace:'choice-mark',
      stem:'יובל סרטטה גרף של פונקציה קווית עולה. הגרף עובר דרך `(2,3)` ובנקודה נוספת. סמנו את הנקודה הנוספת האפשרית.',
      choices:['`(0,5)`','`(3,7)`','`(4,2)`','`(1,3)`'],
      sourceRef:'drive:1JBzDY_ntI7Or2mqAVT1secDAO1FVd2Q4, מקבץ פונקציה קוית מייצב תשעא-תשעו.docx, question 8 — increasing line through (2,3), choose second point from four options',
      adaptation:'ארבע הנקודות המקוריות נשמרו ללא שינוי.'
    },
    {
      id:'MZ08-P67-Q3',family:'MZ08',level:6,responseSpace:'mixed',
      stem:'נתונה הפונקציה `y=-4x+8`.',
      subparts:[
        {text:'סמנו את הפונקציה שהגרף שלה מקביל לגרף הנתון.',responseSpace:'choice-mark',choices:['`y-4x=12`','`y+4x=12`','`y=4x+8`','`y=4x-8`']},
        {text:'סדרו את המשוואה שבחרתם לצורה `y=mx+b`.',responseSpace:'equation'},
        {text:'הסבירו את הבחירה בעזרת השיפועים.',responseSpace:'lines-2'},
        {text:'כעת, בלי קשר למקבילות: סמנו את כל הפונקציות שהשיפוע שלהן הוא `4` (ייתכנו כמה תשובות).',responseSpace:'choice-mark',choices:['`y-4x=12`','`y+4x=12`','`y=4x+8`','`y=4x-8`']}
      ],
      sourceRefs:[
        'drive:1JBzDY_ntI7Or2mqAVT1secDAO1FVd2Q4, מקבץ פונקציה קוית מייצב תשעא-תשעו.docx, question 16(a) — choose the function parallel to y=-4x+8 and explain',
        'drive:1NP2Lcox8WxgZu6QFJMdLebjSn88gZlll — y=mx+b.pdf, question 9 — mark all functions with slope 4 from the same four equations',
        'drive:12sT65k-HodKxZ3lQfpqc-PkzGHjghACw — y=mx+b.docx, question 9 — mark all functions with slope 4 from the same four equations'
      ],
      adaptation:'אפשרויות המקור נשמרו; נוסף שלב סידור לצורה y=mx+b, ונוספה משימת Drive המקורית לזיהוי כל המשוואות ששיפוען 4 מתוך אותן ארבע אפשרויות.'
    },
    {
      id:'MZ09-P67-Q4',family:'MZ09',level:7,responseSpace:'mixed',
      stem:'נקודת החיתוך של `y=-4x+8` עם ציר `y` היא `(0,8)`. כתבו דוגמה לפונקציה קווית עולה שעוברת דרך נקודה זו.',
      subparts:[
        {text:'בחרו שיפוע חיובי.',responseSpace:'short'},
        {text:'כתבו את המשוואה `y=mx+b` המתאימה.',responseSpace:'equation'},
        {text:'בדקו של־`x=0` מתאים `y=8`.',responseSpace:'lines-2'}
      ],
      sourceRef:'drive:1JBzDY_ntI7Or2mqAVT1secDAO1FVd2Q4, מקבץ פונקציה קוית מייצב תשעא-תשעו.docx, question 16(b) — give an increasing linear function through the y-intercept of y=-4x+8',
      adaptation:'המשימה הפתוחה נשמרה; נוספו שלבי בחירת שיפוע ובדיקה בשפה הפדגוגית המחייבת.'
    }
  ]
};
